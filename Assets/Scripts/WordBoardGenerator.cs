...
public class WordBoardGenerator
{
    ...
    public SectionWords GenerateSection(int sectionIndex, GameDataQueueField<string> alreadySeenWords, out string letters)
    {
        ...
        var sortedUppercaseLetters = ChooseCandidateWordsAndLetters(alreadySeenWords.Items, out var candidateWords);
    ...
    private string ChooseCandidateWordsAndLetters(IReadOnlyCollection<string> alreadySeenWords,
        out Queue<string> resultSortedCandidateWords)
    {
                ...
                foreach (var positionCandidate in tilesInSectionByLetter[letter]
                             ...
                             .Where(positionCandidate =>
                                 TryPlaceWordAtPosition(word, tilesInSectionByLetter,
                                     new WordPlacement(positionCandidate, direction),
                                     sectionStartCoordinate, sectionEndCoordinate)))
                {
                    placement = new WordPlacement(positionCandidate, direction);
                    return true;
                }
            }
        }
        ...
        out WordPlacement placement)
    {
        ...
        foreach (var direction in new[] { firstDirection, secondDirection })
        {
            for (var i = 0; i < MaximumFreeWordPlacementAttempts; i++)
            {
                ...
                var placementCandidate = new WordPlacement(positionCandidate, direction);
